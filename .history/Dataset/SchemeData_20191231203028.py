...
class SchemeData():
    ...
    def set_gender_scheme(self, 
        ...
        for gender in ['male', 'femal']:
            hdf5_gender = hdf5_handler('/home/ai/data/yaoyao/Data/DCAE_scheme_{:s}.hdf5'.format(gender).encode)
            gender_group = scheme_group.require_group(gender)
            for fold_name in parcellation:
                output_group = hdf5_gender.require_group('scheme CNNWithGLasso/ABIDE/pearson correlation/{:s}'.format(fold_name))
                ...
                for name in fold_data:
                    create_dataset_hdf5(group=output_group,
                                        data=fold_data[name],
                                        name=name)
