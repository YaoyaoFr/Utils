...
class SchemeData():
    ...
    def set_gender_scheme(self, 
        ...
        for gender in ['male', 'femal']:
            ...
            for fold_name in parcellation:
                XXY_fold_group = hdf5['scheme CNNWithGLasso/ABIDE/pearson correlation/fold {:d}'.format(f)]
                ...
                for p in parcellation[fold_name]:
                    ...
                    fold_data['{:s} data'.format(p)] = np.expand_dims(
                        data[feature_str], axis=-1)[gender_indexes]
                    fold_data['{:s} label'.format(p)] = data[label_str][gender_indexes]
