...
from Dataset.utils import hdf5_handler, create_dataset_hdf5, data_normalization_fold, set_diagonal_to_zero, data_normalization
        ...
        for fold_name in parcellation:
            ...
            for p in parcellation[fold_name]:
                ...
                if 'SmallWorld' in scheme:
                    fold_data['{:s} data'.format(p)] = np.expand_dims(set_diagonal_to_zero(data['pearson correlation XXY']),
                                                                      axis=-1)
                else:
                    fold_data['{:s} data'.format(p)] = np.expand_dims(
                        data['pearson correlation XXY'], axis=-1)
                fold_data['{:s} label'.format(p)] = data['label']
    ...
    def set_DTL_scheme(self,
        ...
        for fold_name in parcellation:
            ...
            for p in parcellation[fold_name]:
                ...
                if 'valid' in p:
                    pearson_correlation = pearson_correlation[np.argmax(
                        label, axis=1) == 1]
                    label = label[np.argmax(label, axis=1) == 1]
