...
def hdf5_handler(filename, mode="r"):
    while True:
        try:
            ...
            print('Loading file {:} error, pause 3 seconds and reexcution.'.format(filename))
            time.sleep(3)
            continue
...
def set_diagonal_to_zero(data: np.ndarray):
    [sample_size, node_nums, node_nums] = np.shape(data)
    for index_sample in range(sample_size):
        for index_node in range(node_nums):
...
def data_normalization_fold(data_fold: h5py.Group or dict,
...
def cal_ROISignals(func_preproc: np.ndarray,
                   atlas: np.ndarray):
    # Get shape
    ...
    for roi_index in range(roi_num):
        signals[:, roi_index] = np.mean(func_preproc[np.where(atlas == roi_index + 1)], axis=0)
